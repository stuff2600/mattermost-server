...
func (a *App) GetChannelsWithChannelContextPermission(userId string, permissionName string) (*model.ChannelList, *model.AppError) {
	if result := <-a.Srv.Store.Channel().GetChannelsWithChannelContextPermission(userId, permissionName); result.Err != nil {
		return nil, result.Err
	} else {
		return result.Data.(*model.ChannelList), nil
	}
